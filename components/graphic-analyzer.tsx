...
/**
 * CARTILHA CROMÁTICA ADAPTA v2026.2 - 39 CORES OFICIAIS
 * 5 famílias: Neutral, OG Hybrid Blend, Linalool Sky, Myrcene Soul, Neutros UI
 */
const CARTILHA_CROMATICA = {
  // Neutral System (tons neutros sem verde)
  neutral: {
    '000': '#F5F7F6',
    '100': '#E8EBE9',
    '200': '#D1D6D4',
  ...
  for (const [espectro, tons] of Object.entries(CARTILHA_CROMATICA)) {
    for (const [tom, hex] of Object.entries(tons)) {
      ...
      if (dist < menorDistancia) {
        menorDistancia = dist;
        corMaisProxima = hex;
        const espectroNome = ESPECTRO_LABELS[espectro] ?? (espectro.charAt(0).toUpperCase() + espectro.slice(1));
        nomeCorMaisProxima = `${espectroNome} ${tom}`;

...
function CoreActMacroDashboard({ userName }: { userName: string }) {
  ...
  const stats = useMemo(() => {
    ...
    const alerts: { id: string; title: string; desc: string; icon: React.ElementType; type: string }[] = [];
...
export default function CoreActOverview() {
  ...
  if (isLoadingRole || authState.type === "loading") {
    return (
      <div style={{ padding: "var(--spacing-5)" }}>
        <Skeleton style={{ height: 100 }} />
        <Skeleton style={{ height: 300, marginTop: "var(--spacing-4)" }} />
      </div>
    );
  }
  ...
  return (
    <div className={styles.coreactContainer}>
      ...
      <CoreActMacroDashboard userName={authState.user.displayName} />
    </div>
  );
}